...
(* here the precondition is saying that we want to read sth and it's saying that k is what we should do next - after reading *)
Definition getchar_pre (m : mem) (witness : byte -> IO_itree) (z : IO_itree) :=
  let k := witness in (sutt (* equivalent even if they don't have exactly the same pieces in them *) eq (r <- read stdin;; k r) z).
